...
export default function TasksPage() {
  ...
  const salvarTarefas = (novaLista: Tarefa[]) => {
  ...
  const allTags = Array.from(
    new Set(
      tarefas.flatMap((t) => t.tags ?? []).filter((t) => t && t.trim() !== "")
    )
  );
            ...
            {tarefas.length > 0 && (
              <section className="flex w-full items-center gap-4 mb-4">
                <select
                  ...
                  className="px-3 h-40 rounded-lg border border-[#212528] bg-white text-black text-sm"
                >
                ...
                <button
                  ...
                  className="h-40 px-6 bg-[#C3EC1D] text-black font-medium text-base rounded-lg hover:opacity-90 transition w-auto"
                >
                  Adicionar tarefa
                </button>
              </section>
                ...
                {tarefasFiltradas.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 w-full">
                    {tarefasFiltradas.map((tarefa) => (
                      <div
                        key={tarefa.id}
                        className="flex flex-col justify-between p-3 min-h-80 rounded-xl border border-[#303030] bg-[#121212] hover:border-[#C3EC1D] transition shadow max-w-xs w-full mx-auto"
                      >
                        <div className="flex items-center mb-1">
                          <h2 className="text-[#C3EC1D] font-medium text-base truncate">
                            {tarefa.titulo}
                          </h2>
                          <button
                        ...
                        <div className="flex flex-wrap gap-1 mt-2">
                          {tarefa.tags.map((tag, idx) => (
                            <span
                              key={idx}
                              className="bg-[#C3EC1D]/30 text-[#C3EC1D] text-xs px-2 py-0.5 rounded-full"
                            >
                              {tag}
                            </span>
                          ))}
